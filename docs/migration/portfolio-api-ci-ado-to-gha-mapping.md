...
| 1 | `JavaToolInstaller@0` `versionSpec=17`, `jdkArchitectureOption=x64`, `jdkSourceOption=PreInstalled` | `actions/setup-java@v4` `distribution=temurin`, `java-version=17`, `cache=maven` | PreInstalled JDK on hosted agent → Temurin from setup-java. Maven dependency cache added (no ADO equivalent) |
| 2 | `Maven@4` `mavenPomFile=pom.xml`, `goals=package`, `options=-B -DskipTests=false` | `run: mvn -f pom.xml $MAVEN_OPTIONS $MAVEN_GOAL` with `working-directory: services/portfolio-api` | See gap **G1** on POM location |
...
| 4 | `script` "Stage build artifacts" (`cp target/*.jar|*.war $(Build.ArtifactStagingDirectory)/`) | same `cp` into `$RUNNER_TEMP/staging` with `mkdir -p` first | `2>/dev/null \|\| true` preserved verbatim |
| 5 | `PublishBuildArtifacts@1` `pathToPublish=$(Build.ArtifactStagingDirectory)`, `artifactName=portfolio-api-dist` | `actions/upload-artifact@v4` `name=portfolio-api-dist`, `path=$RUNNER_TEMP/staging`, `if-no-files-found: error` | `error` makes an empty artifact fail the job (ADO silently published an empty folder) |
| 6 | `script` "Register artifact in artifact-registry" → `publish_artifact.py --registry artifact-registry` | "Register artifact in Artifactory" → `publish_artifact.py --registry Artifactory`, `if: github.event_name == 'push'` | Registry name mapped `master`→`main` (gap **G3**). Push-only guard prevents PR builds registering artifacts |
...
| ID | Gap |
|---|---|
| G1b | **POM location.** The template runs `mvn -f pom.xml` from `$(Build.SourcesDirectory)` (repo root). No `pom.xml` exists anywhere in this repo (no portfolio-api source is checked in). The workflow assumes the service lives in `services/portfolio-api/` (`working-directory`) to match the monorepo layout used by pricing-engine. If the real source is a separate repo or at the root, adjust `SERVICE_DIR`. |
...
| G8 | **Empty artifact now fails.** `if-no-files-found: error` on the dist upload; ADO would publish an empty artifact. Baseline expects 4 files (`.jar`, `.war`, `.xml`) — the `.xml` (e.g. pom) is *not* staged by the template, so the baseline count may already be wrong for ADO. |
